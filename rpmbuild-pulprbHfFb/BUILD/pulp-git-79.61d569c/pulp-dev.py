...
def get_paths_to_copy():
    ...
    paths = [
        ...
        {'source': 'server/pulp/server/webservices/settings.py.example',
         'destination': 'server/pulp/server/webservices/settings.py', 'owner': 'root',
         'group': 'root', 'mode': '755', 'overwrite': True},
    ]
...
def generate_django_settings():
    """
    Create a settings file required for Django. This is required so that the settings file can be
    created with a unique secret key that is not under version control.
    """

    print('Generating Django settings file')
    chars = 'abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*(-_=+)'
    secret = get_random_string(50, chars)
    settings = 'server/pulp/server/webservices/settings.py'

    with open(settings + '.example') as infile, open(settings, 'w') as outfile:
        outfile.write(infile.read().replace('replace-with-secret-key', secret))
...
def install(opts):
    ...
    # Create a new django settings file with a new django secret
